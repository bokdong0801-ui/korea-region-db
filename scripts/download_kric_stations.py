...
def main():
    ...
    req=urllib.request.Request(URL,headers={'User-Agent':'Mozilla/5.0','Referer':DETAIL})
    with urllib.request.urlopen(req,timeout=120) as r:
        raw=r.read(); ctype=r.headers.get('content-type','')
    if len(raw)<10000: raise SystemExit(f'KRiC download unexpectedly small: {len(raw)}')
    out=a.out_dir/f'kric_urban_stations_{a.snapshot_date}.xlsx'; out.write_bytes(raw)
    meta={'source_id':'kric_urban_station','detail_url':DETAIL,'download_url':URL,'snapshot_date':a.snapshot_date,'retrieved_at_utc':datetime.now(timezone.utc).isoformat(),'filename':out.name,'bytes':len(raw),'sha256':sha256(raw),'content_type':ctype}
